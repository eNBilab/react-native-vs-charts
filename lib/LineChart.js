...
class Line extends Component {
  ...
  render() {
    ...
    let toY = Math.floor(this.state.layout.height * this.props.toRatio);
    ...
    let drawToPoint = this.props.pointMode === 'to' || this.props.pointMode === 'both';
    if (drawToPoint) {
      let toPointStyle = {
        ...pointStyle,
...
class LineChart extends Component {
  ...
  _getRatio = (fromValue, toValue) => {
    ...
    let toRatio = (toValue - min) / (max - min);
  ...
  _renderLine = (value, index, values, dataset) => {
    ...
    let toIndex = index + 1;
    let positionMode = toIndex === values.length - 1 ? 'both' : 'from';
